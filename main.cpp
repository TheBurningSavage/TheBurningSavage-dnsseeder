...
public:
  ...
  void ParseCommandLine(int argc, char **argv) {
    static const char *help = "TheBurningSavage-seeder\n"
  ...
  do {
        ...
        count++;
    {
      ...
      for (vector<CAddrReport>::const_iterator it = v.begin(); it < v.end(); it++) {
        CAddrReport rep = *it;
        if (time(NULL)-rep.lastSuccess<24*60*60) activenodes++;
      ...
      fclose(ff);
      ff = fopen("/var/www/html/activenodescount.log", "w");
      fprintf(ff,"%i\n", activenodes);
      fclose(ff);
...
static const string mainnet_seeds[] = {"37.59.63.61", "37.59.63.40", ""};
static const string testnet_seeds[] = {"37.59.63.61", "37.59.63.40", ""};
static const string *seeds = mainnet_seeds;

extern "C" void* ThreadSeeder(void*) {
  if (!fTestNet){
    db.Add(CService("kjy2eqzk4zwi5zd3.onion", 5723), true);
  }
  do {
